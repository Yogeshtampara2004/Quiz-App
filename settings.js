...
class Settings {
  ...
  async startQuiz() {
    try {
      ...
      // Validate if the API returned valid results
      if (!data.results || data.results.length === 0) {
        throw new Error('No questions found. Please try again with different settings.');
      }
  ...
  async fetchData(url) {
    ...
    // Handle network issues or bad responses
    if (!response.ok || !result) {
      throw new Error('There was an error fetching quiz data.');
    }
  ...
  getCurrentDifficulty() {
    ...
    if (checkedDifficulty.length === 1) {
      return checkedDifficulty[0].id; // easy, medium, or hard
    } else {
  ...
  getAmount() {
    ...
    // Validate that the amount is not negative, not 0, and not over 50
